...
def save_pic(pic_path):
    if pic_path.find('bg')>-1:
        pic_name='./geetest/bg.png'
    elif pic_path.find('slice')>-1:
        pic_name='./geetest/slice.png'
    else:
        pic_name='./geetest/fullbg.png'
...
def bg_true(pic_read):
    #pic_read   读取的图片
    pic_write='./geetest/'+pic_read.split('.')[0]+'_true.'+pic_read.split('.')[1]  #保存转换后的图片
    #转换的逻辑顺序
    img = Image.open('./geetest/'+pic_read)
...
def get_distance(pic_read,pic_read1):
    #pic_read   # 读取背景图片
    #pic_read1  # 读取缺口图片
    pic_write='./geetest/match.png'   # 保存匹配最优图
...
def get_validate():
    ...
    distance_s = get_slider_offset_method('./geetest/bg_true.png','./geetest/fullbg_true.png') #计算缺口距离方法1
    # bg_true_pic = 'bg_true.png'    #背景缺口还原图
    # slice_pic = 'slice.png'    #滑块图
    # distance_s = get_distance(bg_true_pic,slice_pic)        #计算缺口距离方法2
    t = get_trace(distance_s,'./geetest/bg_true.png')     #移动轨迹数据方法1
    ...
    with open('./geetest/w.js','r',encoding='utf-8') as f:
        js_code = f.read()
    ...
    if success == '1':
        validate = re.findall('"validate": "(.*?)"',result)[0]
        return validate
